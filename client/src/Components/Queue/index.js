...
const Queue = (props) => {
  ...
  const ifItemsInQueue = () => {
    return (
      <List style={{ width: '100%', height: '100%', overflow: 'none' }}>
        <Grid style={{ padding: '10px' }} container direction="column" alightItems="stretch">
          ...
          {theDbValue.map((el) => {
            ...
            // )
            return (
              <QueueItem firebase={props.firebase} id={itemKey} element={el} isPublic={props.isPublic} key={itemKey} curQueue={props.dbValue} curShop={props.curShop} />
            )
